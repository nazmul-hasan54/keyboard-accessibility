...
export class ProductListWithTableService {

  private productData: ProductCategoryList[] = [
  ...
  getProductByType(typeId: number): ProductCategoryList[]{
    return this.productData.filter(x => x.typeId === typeId);
  }

  getProductByParentAndType(typeId: number, parentId: number): ProductCategoryList[]{
